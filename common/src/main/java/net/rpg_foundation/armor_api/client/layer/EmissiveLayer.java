...
public class EmissiveLayer implements ArmorRenderLayer {
    ...
    /// Armory ships 2.5.
    ///
    /// The shader-color multiplier that used to carry the gain is gone (1.21.11); the burn is
    /// now submitted `floor(gain)` times at full tint plus once at the fractional tint, which
    /// sums to exactly the same `gain × texel` under additive blending.
    ...
    /// Keyed on the (base, mask) pair: with per-stack overrides one mask can sit on several base textures.
    ...
    protected @Nullable Identifier emissiveTexture(ArmorRenderContext context) {
        // Mask resolution: the stack's glowmask override, the constructor's explicit mask, the
