...
///
/// Parsed once per `CustomData` instance (identity-cached; the component object is immutable
...
public record ArmorOverrides(
    ...
    /// Weak identity keys: a `CustomData` compares by content, but the instance on a stack is
    /// replaced on every change, so identity is the cheap and correct cache key; entries die
    /// with the stack.
    private static final LoadingCache<CustomData, ArmorOverrides> CACHE = CacheBuilder.newBuilder()
            .weakKeys()
            .maximumSize(4096)
    ...
    private static ArmorOverrides parse(CustomData data) {
        var overrides = data.copyTag().getCompound(KEY).orElse(null);
        if (overrides == null) {
            return NONE;
        }
    ...
    private static @Nullable Identifier identifier(CompoundTag compound, String key) {
