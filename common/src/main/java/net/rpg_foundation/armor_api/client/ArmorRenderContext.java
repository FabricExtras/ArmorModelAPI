...
public final class ArmorRenderContext {
    ...
    public ArmorRenderContext(
        ...
        this.glintPending = stack.hasFoil() && stack.has(DataComponents.TRIM) && rendersTrims(state);
    }
    ...
    /// baby armor meshes carry none), except on small armor stands - same rule here, so a trimmed
    /// foil piece on a baby also takes the untrimmed glint path. The proportions themselves need
    /// no special case: the pose copy carries the baby body model's baked scale and offsets.
    public boolean rendersTrims() {
        return rendersTrims(state);
    }

    private static boolean rendersTrims(HumanoidRenderState state) {
    ...
    private void submit(RenderType layer, int light, int color, @Nullable UvMapping uvMapping, int outlineColor) {
        queue.order(nextOrder++)
                .submitModel(posedModel, state, matrices, layer, light, OverlayTexture.NO_OVERLAY, color, uvMapping, outlineColor);
    }
}
